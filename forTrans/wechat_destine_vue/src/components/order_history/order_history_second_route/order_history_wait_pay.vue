<template>
    <div class="order-history-all" ref="historyWaitPayScrollHook">
        <div>
            ...
            <div class="order-history-all-noKtvPic-wrapper" v-show="orderHistoryWaitPay.length===0&&!showRefresh&&hasReq">
            ...
            <div class="order-history-all-item" v-for="(order,index) in orderHistoryWaitPay" @click="toOrderDetail(order)">
            ...
            <div class="load-wrapper" v-show="orderHistoryWaitPay.length>0">
...
//			console.log(document.documentElement.clientHeight)
			...
			this.$nextTick(() => {
				this.historyWaitPayScroll = new BScroll(this.$refs.historyWaitPayScrollHook, {
					click: true,
					probeType: 3
				})
				this.historyWaitPayScroll.on('touchend', (pos) => {
				...
				this.historyWaitPayScroll.on('scroll', (pos) => {
					this.y = pos.y
				})
				this.refreshOrderHistory()
			})
...
//				        this.$store.dispatch('setOrderHistory', [])
						this.$store.dispatch('setOrderHistoryWaitPay', this.orderHistoryWaitPay.concat(response.data.rows))
						this.refresh()
						this.$store.dispatch('setIsLoading', false)
						setTimeout(() => {
					...
					if (response.code !== '0') {
						this.$store.dispatch('setIsLoading', false)
						if (this.orderHistoryWaitPay.length > 0) {
			...
			refreshOrderHistory () {
				this.$store.dispatch('setOrderHistoryWaitPay', [])
				this.page2 = 1
				this.requestOrderHistory()
			},
			refresh () {
				this.$nextTick(() => {
					this.historyWaitPayScroll.refresh()
				})
			},
			toOrderDetail (order) {
		...
		computed: {
			...
			view () {
				return this.$refs.historyWaitPayScrollHook.clientHeight
			},
			slider () {
				if (this.orderHistoryWaitPay.length * 172 + 78 < this.view) {
					return this.view + 1
				}
				return this.orderHistoryWaitPay.length * 172 + 78
			},
			heightOfBottom () {
				let allHeight = document.documentElement.clientHeight
				let heightOfBottom
				let viewHeight
				viewHeight = this.orderHistoryWaitPay.length * 172 + 77.6
//                heightOfBottom = allHeight - viewHeight - 44 + 15
			...
			orderHistoryWaitPay () {
				return this.$store.state.orderHistoryWaitPay
			},
			tip () {
				if (this.showLoading) {
					return '正在加载'
