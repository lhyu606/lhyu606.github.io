<template>
    <div class="order-history-all" ref="historyNoUseScrollHook">
        <div>
            ...
            <div class="order-history-all-noKtvPic-wrapper" v-show="orderHistoryNoUse.length===0&&!showRefresh&&hasReq">
            ...
            <div class="order-history-all-item" v-for="(order,index) in orderHistoryNoUse" @click="toOrderDetail(order)">
            ...
            <div class="load-wrapper" v-show="orderHistoryNoUse.length>0">
...
//			console.log(document.documentElement.clientHeight)
			...
			this.$nextTick(() => {
				this.historyNoUseScroll = new BScroll(this.$refs.historyNoUseScrollHook, {
					click: true,
					probeType: 3
				})
				this.historyNoUseScroll.on('touchend', (pos) => {
				...
				this.historyNoUseScroll.on('scroll', (pos) => {
					this.y = pos.y
				})
				this.refreshOrderHistory()
			})
...
//				        this.$store.dispatch('setOrderHistory', [])
						this.$store.dispatch('setOrderHistoryNoUse', this.orderHistoryNoUse.concat(response.data.rows))
						this.refresh()
						this.$store.dispatch('setIsLoading', false)
						setTimeout(() => {
					...
					if (response.code !== '0') {
						this.$store.dispatch('setIsLoading', false)
						if (this.orderHistoryNoUse.length > 0) {
			...
			refreshOrderHistory () {
				this.$store.dispatch('setOrderHistoryNoUse', [])
				this.page1 = 1
				this.requestOrderHistory()
			},
			refresh () {
				this.$nextTick(() => {
					this.historyNoUseScroll.refresh()
				})
			},
			toOrderDetail (order) {
		...
		computed: {
			...
			view () {
				return this.$refs.historyNoUseScrollHook.clientHeight
			},
			slider () {
				if (this.orderHistoryNoUse.length * 172 + 78 < this.view) {
					return this.view + 1
				}
				return this.orderHistoryNoUse.length * 172 + 78
			},
			heightOfBottom () {
				let allHeight = document.documentElement.clientHeight
				let heightOfBottom
				let viewHeight
				viewHeight = this.orderHistoryNoUse.length * 172 + 77.6
//                heightOfBottom = allHeight - viewHeight - 44 + 15
			...
			orderHistoryNoUse () {
				return this.$store.state.orderHistoryNoUse
			},
			tip () {
				if (this.showLoading) {
					return '正在加载'
