<template>
    <div class="order-history-all" ref="historyReturnScrollHook">
        <div>
            ...
            <div class="order-history-all-noKtvPic-wrapper" v-show="orderHistoryReturn.length===0&&!showRefresh&&hasReq">
            ...
            <div class="order-history-all-item" v-for="(order,index) in orderHistoryReturn" @click="toOrderDetail(order)">
            ...
            <div class="load-wrapper" v-show="orderHistoryReturn.length>0">
...
//			console.log(document.documentElement.clientHeight)
			...
			this.$nextTick(() => {
				this.historyReturnScroll = new BScroll(this.$refs.historyReturnScrollHook, {
					click: true,
					probeType: 3
				})
				this.historyReturnScroll.on('touchend', (pos) => {
				...
				this.historyReturnScroll.on('scroll', (pos) => {
					this.y = pos.y
				})
				this.refreshOrderHistory()
			})
...
//				        this.$store.dispatch('setOrderHistory', [])
						this.$store.dispatch('setOrderHistoryReturn', this.orderHistoryReturn.concat(response.data.rows))
						this.refresh()
						this.$store.dispatch('setIsLoading', false)
						setTimeout(() => {
					...
					if (response.code !== '0') {
						this.$store.dispatch('setIsLoading', false)
						if (this.orderHistoryReturn.length > 0) {
			...
			refreshOrderHistory () {
				this.$store.dispatch('setOrderHistoryReturn', [])
				this.page3 = 1
				this.requestOrderHistory()
			},
			refresh () {
				this.$nextTick(() => {
					this.historyReturnScroll.refresh()
				})
			},
			toOrderDetail (order) {
		...
		computed: {
			...
			view () {
				return this.$refs.historyReturnScrollHook.clientHeight
			},
			slider () {
				if (this.orderHistoryReturn.length * 172 + 78 < this.view) {
					return this.view + 1
				}
				return this.orderHistoryReturn.length * 172 + 78
			},
			heightOfBottom () {
				let allHeight = document.documentElement.clientHeight
				let heightOfBottom
				let viewHeight
				viewHeight = this.orderHistoryReturn.length * 172 + 77.6
//                heightOfBottom = allHeight - viewHeight - 44 + 15
			...
			orderHistoryReturn () {
				return this.$store.state.orderHistoryReturn
			},
			tip () {
				if (this.showLoading) {
					return '正在加载'
