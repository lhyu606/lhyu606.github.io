<template>
    <div class="order-history-all" ref="historyAllScrollHook">
        <div>
            ...
            <div class="order-history-all-noKtvPic-wrapper" v-show="orderHistory.length===0&&!showRefresh&&hasReq">
            ...
            <div class="order-history-all-item" v-for="(order,index) in orderHistory" @click="toOrderDetail(order)">
            ...
            <div class="load-wrapper" v-show="orderHistory.length>0">
...
//			console.log(document.documentElement.clientHeight)
			...
			this.$nextTick(() => {
				this.historyAllScroll = new BScroll(this.$refs.historyAllScrollHook, {
					click: true,
					probeType: 3
				})
				this.historyAllScroll.on('touchend', (pos) => {
				...
				this.historyAllScroll.on('scroll', (pos) => {
					this.y = pos.y
				})
				this.refreshOrderHistory()
			})
...
//				        this.$store.dispatch('setOrderHistory', [])
						this.$store.dispatch('setOrderHistory', this.orderHistory.concat(response.data.rows))
						this.refresh()
						this.$store.dispatch('setIsLoading', false)
						setTimeout(() => {
		...
		computed: {
			...
			view () {
				return this.$refs.historyAllScrollHook.clientHeight
			},
			slider () {
				if (this.orderHistory.length * 172 + 78 < this.view) {
					return this.view + 1
				}
				return this.orderHistory.length * 172 + 78
			},
			heightOfBottom () {
				let allHeight = document.documentElement.clientHeight
				let heightOfBottom
				let viewHeight
				viewHeight = this.orderHistory.length * 172 + 77.6
