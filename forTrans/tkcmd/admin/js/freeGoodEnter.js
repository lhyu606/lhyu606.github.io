...
layui.use(['jquery', 'laydate', 'layer', 'table', 'form'], function () {
    ...
    $("body").on("click",".sure",function() {
        var postInfoArr = ['auctionUrl','pictUrl','title','adTitle','auctionId','shopTitle','sellerId','biz30day',
            'zkPrice','reservePrice','hiddenInfo','hiddenAmount','tkCommFee','tkRate','hiddenStartTime',
            'hiddenEndTime','tag', 'group_buy_num', 'receive_num', 'start_date', 'hiddenActivityId',
            'end_date', 'left_num', 'total_num'];
        ...
        if(onSetGoodsInfo && onSetGoodsInfo.auctionId) {
            ...
            if(onSetGoodsInfo.group_buy_num =='' || parseInt(onSetGoodsInfo.group_buy_num) < 0) {
                layer.msg("比率不能为空！");
                return;
            }
            saveFreeGood(onSetGoodsInfo);
 ...
 * url/tao_goods/save
 */
function saveFreeGood(param) {
    param.isArtificial = 1;
    $.ajax({
        url: server_url + "/boutique_pool/saveFreeGood",
        type: "post",
        dataType: "json",
        data: param,
        success: function (data) {
