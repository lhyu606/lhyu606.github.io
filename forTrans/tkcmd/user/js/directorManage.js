    ...
    $('#table2excel').click(function () {
        ...
        setTimeout(function () {
            $("#excelList").table2excel({
                exclude: ".noExl",
                name: "Worksheet Name",
                filename: "总监列表",//do not include extension
                exclude_img: true,
                exclude_links: true,
                exclude_inputs: true
            });
...
function membersList(param, isInside) {
    $.ajax({
        ...
        success: function(data) {
            if(data.status == 200) {
                $('.rolePlay', parent.document).text('总监');
                ...
                if(data.payload.data) {
                    ...
                    $.each(staffList, function(index,obj) {
                        ...
                        if(obj.team_num > 0) {
                            html += '<a class="layui-btn layui-btn-xs lookNext" '+
                                'account="'+ obj.account +'" title="点击查看下线详情">查看下级</a>'
                        } else {
